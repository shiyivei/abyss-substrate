...
pub mod pallet {
	...
	pub type Pov2uc<T: Config> = StorageValue<_, u8>;

	#[pallet::storage]
	#[pallet::getter(fn uc2gametime)]
	pub type Uc2gametime<T: Config> = StorageValue<_, u8>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn exchangepov2uc(
			...
			match res {
				Ok(amount_u128) => {
					let target_balance: Result<BalanceOf<T>, _> = (amount_u128 / 10).try_into();
